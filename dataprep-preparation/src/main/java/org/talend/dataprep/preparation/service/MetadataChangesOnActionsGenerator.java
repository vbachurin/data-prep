...
public class MetadataChangesOnActionsGenerator {
    ...
    StepDiff computeCreatedColumns(List<RunnableAction> newActions, RowMetadata reference) {
        ...
        List<String> createdColumnIds = updatedMetadata.getColumns().stream()
                .filter(c -> Flag.NEW.getValue().equals(c.getDiffFlagValue())).map(ColumnMetadata::getId).collect(toList());
