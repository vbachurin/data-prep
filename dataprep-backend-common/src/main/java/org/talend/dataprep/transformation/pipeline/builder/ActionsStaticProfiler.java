...
public class ActionsStaticProfiler {
     ...
     */
    public Map<Action, ActionMetadata> getActionMetadataByAction(final List<Action> actions) {
        final Map<Action, ActionMetadata> actionToMetadata = new HashMap<>(actions.size());
    ...
    public ActionsProfile profile(final List<ColumnMetadata> columns, final List<Action> actions, final Map<Action, ActionMetadata> actionToMetadata) {
        final Map<Action, ActionMetadata> metadataByAction = actionToMetadata == null ?
                getActionMetadataByAction(actions) :
                actionToMetadata;

        // Compile actions
        final Set<String> originalColumns = columns
                .stream()
        ...
        for (Map.Entry<Action, ActionMetadata> entry : metadataByAction.entrySet()) {
