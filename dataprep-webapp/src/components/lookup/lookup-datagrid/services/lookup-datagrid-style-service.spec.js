describe('Lookup Datagrid style service', function () {
	...
	describe('on active cell changed event', function () {
		...
		beforeEach(function () {
			...
			stateMock.playground.lookupGrid.dataView = dataViewMock;
		});

		beforeEach(inject(function(DatagridService) {
			//spyOn(stateMock.playground.lookupGrid.dataView, 'getItem').and.returnValue({'0001': 'cell 1 content'});
	...
	describe('column preview style', function() {
		it('should return "new" column style', inject(function (LookupDatagridStyleService) {
			//given
			LookupDatagridStyleService.init(gridMock);
			var col = {__tdpColumnDiff: 'new'};

			//when
			var diffClass = LookupDatagridStyleService.getColumnPreviewStyle(col);

			//then
			expect(diffClass).toBe('newColumn');
		}));

		it('should return "deleted" column style', inject(function (LookupDatagridStyleService) {
			//given
			LookupDatagridStyleService.init(gridMock);
		...
		it('should return "updated" column style', inject(function (LookupDatagridStyleService) {
			//given
			LookupDatagridStyleService.init(gridMock);
		...
		it('should return empty string on no change diff', inject(function (LookupDatagridStyleService) {
			//given
			LookupDatagridStyleService.init(gridMock);
			var col = {};
