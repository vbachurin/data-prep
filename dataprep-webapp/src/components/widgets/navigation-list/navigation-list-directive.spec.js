describe('navigationList directive', function() {
	...
	describe('when list has 3 items and number of shown labels are 4', function() {
		...
		beforeEach(inject(function ($rootScope, $compile) {
			...
			createElement = function () {
				element          = angular.element('<navigation-list ' +
					...
					'selected-item="item"' +
					'get-label="getLabelCb(item)"' +
					'nbre-labels-to-show="4"></navigation-list>'
				);
				scope.list       = list;
				scope.item       = list[0];
				scope.getLabelCb = function (item) {
		...
		it('should disable arrow-left and arrow-right buttons', function () {
			//when
			createElement();
