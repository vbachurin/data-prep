describe('Transformation choice params directive', function () {
    ...
    it('should render an action with simple choice', function() {
        ...
        //then
        expect(element.find('.param-choice-name').length).toBe(1);
        expect(element.find('.param-choice-name').eq(0).text().trim()).toBe('my choice :');
        expect(element.find('.param-choice-select').length).toBe(1);
        expect(element.find('.param-choice-select').eq(0).find('select').length).toBe(1);
        expect(element.find('.param-choice-select').eq(0).find('option').length).toBe(2);
        expect(element.find('.param-choice-select').eq(0).find('option').eq(0).text()).toBe('noParamChoice1');
        expect(element.find('.param-choice-select').eq(0).find('option').eq(1).text()).toBe('noParamChoice2');
    });
