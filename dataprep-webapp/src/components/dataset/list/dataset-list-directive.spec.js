describe('Dataset list directive', function() {
    ...
    beforeEach(module('pascalprecht.translate', function ($translateProvider) {
        $translateProvider.translations('en', {
            'INVENTORY_DETAILS': "owned by {{author}}, created {{created | moment_from_now:'D-M-YYYY h:m'}}, contains {{records}} lines"
        });
    ...
    it('should render dataset list', function() {
        ...
        var momentAgo = moment('02-03-2015 14:54','D-M-YYYY h:m').fromNow();
        ...
        expect(element.find('.inventory-description').eq(0).text()).toBe('owned by anonymousUser, created '+momentAgo+', contains  lines');
        ...
        expect(element.find('.inventory-description').eq(1).text()).toBe('owned by anonymousUser, created '+momentAgo+', contains  lines');
        ...
        expect(element.find('.inventory-description').eq(2).text()).toBe('owned by anonymousUser, created '+momentAgo+', contains  lines');
    });
});
