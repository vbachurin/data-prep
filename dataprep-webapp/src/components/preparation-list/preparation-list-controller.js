(function() {
    ...
    function PreparationListCtrl($rootScope, $stateParams, PlaygroundService, PreparationService, TalendConfirmService, MessageService, StateService) {
        ...
        /**
         * @ngdoc method
         * @name nameInputKeyPressed
         * @methodOf data-prep.dataset-list.controller:PreparationListCtrl
         * @description handle keypressed in the input test
         * @param {object) event - the event generated by the key pressed
         * @param {object} dataset - the preparation
         */
        vm.nameInputKeyPressed = function(event,preparation){
            // 27 ESC
            // 13 enter
            if (event.keyCode===27){
                vm.cancelRename(preparation);
            }
            if (event.keyCode===13){
                vm.rename(preparation);
            }
        };
