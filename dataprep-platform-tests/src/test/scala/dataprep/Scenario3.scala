...
class Scenario3 extends Simulation {
  val httpConf = http
  ...
  val scn = scenario("Create / Read / Transform Upper Case")
    ...
    group("actions") {
      exec(http("creation").post("api/datasets/").body(RawFileBody("data/test1.csv")).check(bodyString.saveAs("dataset")))
        ...
        .exec(http("preparation_creation").post("api/preparations").header("content-type", "application/json").body(StringBody("{\n  \"id\": \"${dataset}\"\n}"))).check(bodyString.saveAs("preparation")))
